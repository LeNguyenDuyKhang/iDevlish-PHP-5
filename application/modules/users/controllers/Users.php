...
class Users extends Layout
{
    ...
    function ajax_search()
    {
        ...
        if (!empty($post)) {
            ...
            $args['in_role'] = array('MEMBER', 'LEADER', 'INVESTOR');
    ...
    function get_max_shop_id()
    {
        ...
        $rows = $this->M_users->gets($args, 1, 0);
        $shop_id = (int) (isset($rows[0]['shop_id']) ? filter_var($rows[0]['shop_id'], FILTER_SANITIZE_NUMBER_INT) : 0) + 1;
        return $shop_id;
    }
    ...
    function admin_change_shop_status_ajax()
    {
        ...
        if (!empty($post)) {
            ...
            if (!(is_array($row) && !empty($row))) {
                $message['status'] = 'error';
                $message['content'] = null;
                $message['message'] = show_alert_danger('Cửa hàng không tồn tại!');
                echo json_encode($message);
                exit();
            }
            ...
            if ($this->M_users->update($id, $data)) {
                ...
                $message['content'] = array(
                    'status_label' => display_value_array($this->config->item('shop_status_label'), $status),
                    'status_text' => display_value_array($this->config->item('shop_status'), $status),
                    'status_action' => get_shop_action_by_status($status),
                );
                $message['message'] = show_alert_success('Thay đổi trạng thái cửa hàng thành công!');
            } else {
                $message['status'] = 'error';
                $message['content'] = null;
    ...
    function index_shop()
    {
        ...
        if (isset($get['status']) && trim($get['status']) != '') {
            $args['shop_status'] = (int) $get['status'];
        }
        $args['not_in_shop_status'] = array(-1);
        ...
        if (!empty($get)) {
            $config['base_url'] = get_admin_url('users/shop');
            $config['suffix'] = '?' . http_build_query($get, '', "&");
            $config['first_url'] = get_admin_url('users/shop' . '?' . http_build_query($get, '', "&"));
        ...
        $this->_data['breadcrumbs_module_func'] = 'Danh sách cửa hàng';
        $this->_data['title'] = 'Danh sách cửa hàng - ' . $this->_data['title'];;
        $this->_data['main_content'] = 'users/admin/view_page_shop_index';
    ...
    function index()
    {
        ...
        /*
        $rows = $this->M_users->gets(array());
        foreach ($rows as $value) {
            if(trim($value['refer_key']) == ''){
                echo $value['username'] . "<br/>";
                $refer_key = $this->__get_refer_key();
                $this->M_users->update($value['userid'], array(
                    'refer_key' => $refer_key
                ));
            }
        }
    ...
    function admin_content()
    {
        ...
        if ($userid != 0) {
            ...
            $args['status'] = 1;
            $this->_data['products'] = $this->M_shops_rows->search($args);
            // var_dump($this->_data['products']); die;
    ...
    function admin_shop_content()
    {
        ...
        if (!empty($post)) {
            $this->form_validation->set_error_delimiters('<span class="help-block">', '</span>');
            $this->form_validation->set_rules('shop_name', 'Tên cửa hàng', 'trim|required|max_length[255]');
            ...
            if ($this->form_validation->run($this)) {
                $data = array(
                    ...
                    'shop_keywords' => $this->input->post('shop_keywords'),
                    'shop_modified' => time(),
                );
                    ...
                    if (isset($info['uploads'])) {
                        ...
                        if ($_FILES[$input_name]['size'] != 0) {
                            foreach ($upload_images as $value) {
                                ...
                                $this->M_users->update($user_id, $data_images);
                            }
                        }
                    }

                    $notify_type = 'success';
                    $notify_content = 'Cập nhật cửa hàng thành công!';
                    $this->set_notify_admin($notify_type, $notify_content);
                    redirect(get_admin_url($this->_module_slug));
                } else {
                    $notify_type = 'danger';
        ...
        $this->_data['breadcrumbs_module_func'] = 'Cập nhật tài khoản';
        $this->_data['title'] = 'Cập nhật tài khoản - ' . $this->_data['title'];
        $this->_data['main_content'] = 'users/admin/view_page_shop_content';
        $this->load->view('layout/admin/view_layout', $this->_data);
    }
    ...
    function site_register_shop()
    {
        ...
        if (!empty($post)) {
            ...
            if ($this->form_validation->run($this)) {
                ...
                if ($this->M_users->update($user_id, $data)) {
                    ...
                    $data['full_name'] = $this->_data['full_name'];
                    $email = get_first_element($this->_data['email']);
                    $partial = array();
                    $partial['data'] = $data;
                    $receiver_email = array($this->_data['email'], 'lenhan10th@gmail.com');
                    $data_sendmail = array(
                        'sender_email' => $email,
                        'sender_name' => $this->_data['site_name'] . ' - Đăng ký bán hàng - ' . $email,
                        'receiver_email' => $receiver_email,
                        'subject' => 'Đăng ký bán hàng',
                        'message' => $this->load->view('layout/site/partial/html-template-notify-new-shop', $partial, true)
                    );
                    modules::run('emails/send_mail', $data_sendmail);

                    $notify_type = 'success';
                    $notify_content = 'Đăng ký bán hàng thành công! Vui lòng chờ xét duyệt của chúng tôi!';
                    $this->set_notify($notify_type, $notify_content);
                    $this->_data['shop_status'] = 0;
                    // redirect(site_url());
                } else {
                    $notify_type = 'danger';
                    $notify_content = 'Có lỗi xảy ra! Vui lòng thực hiện lại!';
        ...
        $this->_data['title_seo'] = 'Đăng ký bán hàng' . ' - ' . $this->_data['title_seo'];
        $this->_data['main_content'] = 'layout/site/pages/register-shop';
    ...
    function site_register()
    {
        ...
        $this->_data['ref'] = $this->input->get('ref');
