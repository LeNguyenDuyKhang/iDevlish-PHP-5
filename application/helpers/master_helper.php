...
if (!function_exists('get_bank_info')) {
    function get_bank_info($user_id = 0) {
        ...
        if (isset($row['bank_branch']) && trim($row['bank_branch']) != '') {
            $arr_address[] = 'Chi nhánh: ' . $row['bank_branch'];
        }
...
if (!function_exists('get_commission_user')) {
    function get_commission_user($user_id = 0, $not_in_id = 0) {
        ...
        $total_withdrawal = abs($ci->M_users_commission->get_total(array(
            'user_id' => $user_id,
            'status' => 1,
        ...
        $total_commission = abs($ci->M_users_commission->get_total(array(
            'user_id' => $user_id,
            'status' => 1,
            'in_action' => array('SUB_BUY')
        )));

        $total_commission_ref = abs($ci->M_users_commission->get_total(array(
            'user_id' => $user_id,
            'status' => 1,
            'in_action' => array('SUB_BUY_REFERRED_ROOT', 'SUB_BUY_REFERRED')
        )));

        $balance = $total_commission + $total_commission_ref - $total_withdrawal ;

        return array(
            'total_withdrawal' => $total_withdrawal,
            'total_commission' => $total_commission,
            'total_commission_ref' => $total_commission_ref,
            'balance' => $balance,
        );
    }
}
        ...
        if (is_array($data) && !empty($data)) {
            foreach ($data as $value) {
                ...
                if (is_array($option_selected) && in_array($value[$option_value], $option_selected)) {
                    $selected = ' selected="selected"';
                } elseif (strcmp($value[$option_value], $option_selected) === 0) {
                    $selected = ' selected="selected"';
                }
                $html .= "\n";
...
if (!function_exists('get_user_level_percent')) {
    function get_user_level_percent($user_level = ''){
        $CI =& get_instance();
        return (int) display_value_array($CI->config->item('level_percent'), $user_level);
    }
}

if (!function_exists('get_discount_price_by_percent')) {
    function get_discount_price_by_percent($price = 0, $percent = 0){
...
if (!function_exists('get_user_shop_id')) {
    function get_user_shop_id($user_id = 0){
        $CI =& get_instance();
        $user = $CI->M_users->get($user_id);
        return isset($user['shop_id']) ? (int) $user['shop_id'] : NULL;
    }
}

if (!function_exists('get_user_shop_status')) {
    function get_user_shop_status($user_id = 0){
        $CI =& get_instance();
        $user = $CI->M_users->get($user_id);
        return isset($user['shop_status']) ? (int) $user['shop_status'] : NULL;
    }
}

if (!function_exists('get_shop_action_by_status')) {
    function get_shop_action_by_status($status = 0){
        $html = '';
        if($status == 0){
            $html = '<li><a class="dropdown-item shop-status" data-status="1" href="javascript:void(0)"><i class="fa fa-check"></i> Duyệt</a></li>';
        }elseif($status == 1){
            $html = '<li><a class="dropdown-item shop-status" data-status="-2" href="javascript:void(0)"><i class="fa fa-lock"></i> Khóa</a></li>';
        }elseif($status == -2){
            $html = '<li><a class="dropdown-item shop-status" data-status="1" href="javascript:void(0)"><i class="fa fa-unlock"></i> Mở khóa</a></li>';
        }
        return $html;
    }
}
